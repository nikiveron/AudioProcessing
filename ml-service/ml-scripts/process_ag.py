#!/usr/bin/env python3
"""
Скрипт для обработки аудио акустической гитары с помощью улучшенной U-Net модели.
Использование: python process_ag.py <input_audio_path> <output_audio_path>
...
MODEL_PATH = "model_weights_ag_improved_unet.pth"
SAMPLE_RATE = 48000
...
def process_audio(model, input_path, output_path, chunk_size=15, reference_path=None):
    ...
    if len(audio) < chunk_size * SAMPLE_RATE:
        ...
        output_mag = spectral_smoothing(output_mag, smoothing_factor=0.15)
        
        print("  Применяю spectral gating (щадящий)...")
        output_mag = spectral_gating(output_mag, threshold=0.04, floor=0.2)
        
        print("  Применяю частотную коррекцию...")
        output_mag = frequency_weighting(output_mag, low_freq_boost=1.15, high_freq_attenuation=0.92)
        # ============================================================================
        ...
        nyquist = sr / 2
        cutoff = 10000  # 14 kHz для акустической гитары (сохраняем обертоны и воздушность)
...
def main():
    ...
    print("="*70)
    print("Обработка аудио акустической гитары улучшенной U-Net моделью")
    print("="*70)
