...
BASE_DIR = "data"
EG_RAW_DIR = os.path.join(BASE_DIR, "eg", "raw")
EG_PROCESSED_DIR = os.path.join(BASE_DIR, "eg", "processed")

# Директории для сегментов
EG_SEGMENTS_RAW_DIR = os.path.join(BASE_DIR, "eg_segments", "raw")
EG_SEGMENTS_PROCESSED_DIR = os.path.join(BASE_DIR, "eg_segments", "processed")

os.makedirs(EG_SEGMENTS_RAW_DIR, exist_ok=True)
os.makedirs(EG_SEGMENTS_PROCESSED_DIR, exist_ok=True)

print("="*70)
print("Обучение УЛУЧШЕННОЙ U-Net модели обработки ЭЛЕКТРО-ГИТАРЫ")
print("="*70)
...
file_pairs = match_file_pairs(EG_RAW_DIR, EG_PROCESSED_DIR)

if not file_pairs:
    print("\n⚠️ Не найдено пар файлов! Проверяем альтернативный метод сопоставления...")
    
    # Альтернативный метод: просто сопоставляем по порядку
    raw_files = sorted(glob.glob(os.path.join(EG_RAW_DIR, "*.wav")))
    processed_files = sorted(glob.glob(os.path.join(EG_PROCESSED_DIR, "*.wav")))
...
raw_segments = glob.glob(os.path.join(EG_SEGMENTS_RAW_DIR, "*.wav"))
processed_segments = glob.glob(os.path.join(EG_SEGMENTS_PROCESSED_DIR, "*.wav"))
...
if len(raw_segments) > 0 and len(processed_segments) > 0:
    ...
    for raw_file, _ in file_pairs:
        split_and_save(raw_file, EG_SEGMENTS_RAW_DIR, segment_duration=SEGMENT_DURATION, sample_rate=SAMPLE_RATE)

    print("\nОбработка обработанных записей...")
    for _, proc_file in file_pairs:
        split_and_save(proc_file, EG_SEGMENTS_PROCESSED_DIR, segment_duration=SEGMENT_DURATION, sample_rate=SAMPLE_RATE)
...
matched_pairs, unmatched_raw = match_segments_by_name(EG_SEGMENTS_RAW_DIR, EG_SEGMENTS_PROCESSED_DIR)
...
MATCHED_RAW_DIR = EG_SEGMENTS_RAW_DIR + "_matched"
MATCHED_PROCESSED_DIR = EG_SEGMENTS_PROCESSED_DIR + "_matched"
...
model_path = f"model_weights_eg_{model_type_str}.pth"
