...
from app.models.strategies.config import SAMPLE_RATE, MODEL_CONFIG, CHUNK_SIZE, OVERLAP_RATIO, DEVICE, AG_MODEL_PATH, POST_PROCESSING_AG


class AGProcessingStrategy(BaseProcessingStrategy):

    def __init__(self):
        super().__init__(
            model_path=str(AG_MODEL_PATH),
    ...
    def apply_post_processing(self, magnitude: np.ndarray, phase: np.ndarray) -> np.ndarray:
        lowpass_cutoff = POST_PROCESSING_AG.get("lowpass_cutoff", 10000)
        lowpass_order = POST_PROCESSING_AG.get("lowpass_order", 4)
