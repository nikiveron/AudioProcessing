...
from app.models.model_unet_improved import ImprovedUNetSeparator, ImprovedComplexUNetSeparator
from app.models.utils_unet import split_and_save, AudioEffectDataset, SSIMLoss

device = torch.device("cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu")


BASE_DIR = "data"
AG_RAW_DIR = os.path.join(BASE_DIR, "ag", "raw")
AG_PROCESSED_DIR = os.path.join(BASE_DIR, "ag", "processed")

# Директории для сегментов
AG_SEGMENTS_RAW_DIR = os.path.join(BASE_DIR, "ag_segments", "raw")
AG_SEGMENTS_PROCESSED_DIR = os.path.join(BASE_DIR, "ag_segments", "processed")

os.makedirs(AG_SEGMENTS_RAW_DIR, exist_ok=True)
os.makedirs(AG_SEGMENTS_PROCESSED_DIR, exist_ok=True)

print("="*70)
print("Обучение УЛУЧШЕННОЙ U-Net модели обработки акустической гитары")
print("="*70)
...
file_pairs = match_file_pairs(AG_RAW_DIR, AG_PROCESSED_DIR)

if not file_pairs:
    print("\n⚠️ Не найдено пар файлов! Проверяем альтернативный метод сопоставления...")
    
    # Альтернативный метод: просто сопоставляем по порядку
    raw_files = sorted(glob.glob(os.path.join(AG_RAW_DIR, "*.wav")))
    processed_files = sorted(glob.glob(os.path.join(AG_PROCESSED_DIR, "*.wav")))
...
raw_segments = glob.glob(os.path.join(AG_SEGMENTS_RAW_DIR, "*.wav"))
processed_segments = glob.glob(os.path.join(AG_SEGMENTS_PROCESSED_DIR, "*.wav"))
...
if len(raw_segments) > 0 and len(processed_segments) > 0:
    ...
    for raw_file, _ in file_pairs:
        split_and_save(raw_file, AG_SEGMENTS_RAW_DIR, segment_duration=SEGMENT_DURATION, sample_rate=SAMPLE_RATE)

    print("\nОбработка обработанных записей...")
    for _, proc_file in file_pairs:
        split_and_save(proc_file, AG_SEGMENTS_PROCESSED_DIR, segment_duration=SEGMENT_DURATION, sample_rate=SAMPLE_RATE)
...
matched_pairs, unmatched_raw = match_segments_by_name(AG_SEGMENTS_RAW_DIR, AG_SEGMENTS_PROCESSED_DIR)
...
MATCHED_RAW_DIR = AG_SEGMENTS_RAW_DIR + "_matched"
MATCHED_PROCESSED_DIR = AG_SEGMENTS_PROCESSED_DIR + "_matched"
...
model_path = f"model_weights_ag_{model_type_str}.pth"
